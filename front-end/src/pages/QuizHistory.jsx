...
const QuizHistory = () => {
  ...
  if (loading) return <div className="text-white p-6">Loading Quiz History...</div>;
  if (history.length === 0) return <div className="text-white p-6">No Quiz History Found</div>;
      ...
      <table className="w-[95%] m-auto shadow-xl/30">
        ...
        <tbody className="text-center">
          {history.map((quiz, idx) => {
            ...
            return (
              <tr key={idx} className="bg-[#055e58]">
                <td className="px-4 py-2">{`Quiz ${idx + 1}`}</td>
                <td className="px-4 py-2">{submittedAt}</td>
                <td className="px-4 py-2">{result}</td>
              </tr>
            );
